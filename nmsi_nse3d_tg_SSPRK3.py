#!/usr/bin/env python3
"""
NMSI–π*–HDQG augmented incompressible NSE (3D) — Taylor–Green vortex (pseudo‑spectral).
Upgrades in this version:
- SSPRK3 time integrator (replaces forward Euler)
- Spherical 2/3-dealiasing
- Energy budget logging (nu-dissipation, gamma-dissipation, pi* injection)
- 'compare_mode.classical' flag to disable augmentation for baseline NSE

Note: Educational reference; for research-grade work, consider MPI/GPU and higher resolution.
"""
import argparse, os
import numpy as np
import yaml
...
def main():
    ...
    nu = cfg["physics"]["nu"]
    use_dealias = cfg.get("dealias", {}).get("use", True)

    # Forcing and dissipation params
    fcfg = cfg["forcing_pi"]; gcfg = cfg["gamma_diss"]
    A_pi, omega_pi, phi = fcfg["A_pi"], fcfg["omega_pi"], fcfg["phi"]
    k1, k2 = fcfg["k1"], fcfg["k2"]
    use_pi = fcfg.get("use", True)
    use_band = fcfg.get("bandpass", True)

    gamma0 = gcfg["gamma0"]; omegaZ = gcfg["omega_Z"]
    phiZ = gcfg["phiZ"]; zeta = gcfg["zeta"]
    use_gamma = gcfg.get("use", True)

    # Compare mode: classical NSE baseline
    ...
    K2 = KX**2 + KY**2 + KZ**2
    mask, Kmag = spherical_dealias(KX, KY, KZ, frac=2/3) if use_dealias else (np.ones((nx,ny,nz)), np.sqrt(K2))
    ...
    cfl  = cfg["time"]["cfl"]
    dt_min, dt_max = cfg["time"]["dt_min"], cfg["time"]["dt_max"]
    save_every = cfg["time"]["save_every"]
    istep = 0

    # Output
    outdir = cfg.get("io", {}).get("outdir", "./out_3d")
    os.makedirs(outdir, exist_ok=True)

    # Diagnostics
    times, E_list, Omega_list, Wmax_list = [], [], [], []
    InjPi_list, EpsNu_list, EpsGamma_list = [], [], []
    ...
    def RHS(uh, tlocal):
        ...
        # pi* forcing (band-limited, bounded)
        if use_pi:
            shell = shell_band(Kmag, k1, k2) if use_band else np.ones_like(Kmag)
            Fhat = A_pi * np.cos(omega_pi*tlocal + phi) * shell * uh
        else:
            Fhat = 0.0

        # intermittent gamma_diss
        if use_gamma:
            gate = 1.0 if np.sin(omegaZ*tlocal + phiZ) > zeta else 0.0
            gamma_t = gamma0 * gate
        else:
            gamma_t = 0.0
    ...
    while t < t_end:
        ...
        # approximate energy budget pieces
        # gradient magnitude squared ~ sum |∇u|^2 (component-wise)
        gx = grad(u[0]); gy = grad(u[1]); gz = grad(u[2])
        grad_u_sq = sum(gx_i**2 for gx_i in gx) + sum(gy_i**2 for gy_i in gy) + sum(gz_i**2 for gz_i in gz)
        eps_nu = nu * np.mean(grad_u_sq)
        eps_gamma = ( (gamma_t1+gamma_t2+gamma_t3)/3.0 ) * np.mean(u[0]**2 + u[1]**2 + u[2]**2) if use_gamma else 0.0
